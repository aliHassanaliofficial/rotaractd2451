...
const SOCIAL_ICONS = [
  ...
  { key: 'twitter', label: 'Twitter / X', path: 'M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298Z' },
]

export function Footer() {
  const { settings } = useSiteSettings()
  const districtName = settings.district_name || 'Rotaract'
  const contact = settings.contact_info || {}
  const social = settings.social_media || {}
  const logoUrl = settings.logo_url || '/logo-white.png'
  ...
  const socialLinks = SOCIAL_ICONS.map(({ key, path, label }) =>
    social[key] ? { href: social[key], path, label } : null
  ).filter((l): l is { href: string; path: string; label: string } => Boolean(l))

  return (
    <footer className="relative isolate overflow-hidden bg-gradient-to-br from-navy via-[#0a4a82] to-[#0e2f4f] text-white">
      <div className="pointer-events-none absolute -left-24 top-0 -z-10 h-72 w-72 rounded-full bg-cranberry/20 blur-3xl" />
      <div className="pointer-events-none absolute -right-24 bottom-0 -z-10 h-72 w-72 rounded-full bg-gold/10 blur-3xl" />
      <div className="container mx-auto px-4 py-12">
        <div className="grid gap-8 md:grid-cols-4">
          <div>
            ...
            {socialLinks.length > 0 && (
              <div className="mt-4 flex gap-3">
                {socialLinks.map(({ href, path, label }) => (
                  <Link
                    key={label}
                    href={href}
                    target="_blank"
                    ...
                    aria-label={label}
                    title={label}
                    className="rounded-full bg-white/10 p-2 transition-colors hover:bg-cranberry"
                  >
          ...
          <div>
            <h3 className="mb-4 text-sm font-semibold uppercase tracking-wider text-gold">Resources</h3>
            <ul className="space-y-2">
              {[
                { href: '/library', label: 'Library' },
                { href: '/announcements', label: 'Announcements' },
                { href: '/history', label: 'District History' },
          ...
          <div>
            <h3 className="mb-4 text-sm font-semibold uppercase tracking-wider text-gold">Contact</h3>
