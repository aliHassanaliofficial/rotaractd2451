...
async function runSQL(sql: string) {
  // Use pg directly via child_process
  const { execSync } = await import('child_process')
  const connString = `postgresql://postgres.${projectRef}:${encodeURIComponent(dbPassword)}@${poolerHost}:6543/postgres`
  const tempFile = path.join(__dirname, '..', 'tmp_migration.sql')
  fs.writeFileSync(tempFile, sql)
  try {
    execSync(`psql "${connString}" -f "${tempFile}"`, { stdio: 'inherit', timeout: 60000 })
  } catch (e) {
    // Try direct connection
    try {
      execSync(`psql "postgresql://postgres:${encodeURIComponent(dbPassword)}@db.${projectRef}.supabase.co:5432/postgres" -f "${tempFile}"`, { stdio: 'inherit', timeout: 60000 })
    } catch (e2) {
      console.error('Failed to connect via psql. Make sure psql is installed.')
      process.exit(1)
    }
  } finally {
...
async function createStorageBuckets() {
  ...
  const buckets = [
    { id: 'avatars', public: true, name: 'avatars' },
    { id: 'club-logos', public: true, name: 'club-logos' },
    { id: 'event-covers', public: true, name: 'event-covers' },
    { id: 'gallery', public: true, name: 'gallery' },
    { id: 'library', public: true, name: 'library' },
    { id: 'documents', public: false, name: 'documents' },
  ]
...
INSERT INTO site_settings (key, value) VALUES
  ...
  ('feature_flags', '{"registration_open":true,"guest_registration":true,"show_member_directory":true,"gallery_public":true,"maintenance_mode":false}'::jsonb)
...
`
  ...
  try {
    ...
    console.log('psql not found, using pg library directly...')
  }
  ...
  const connString = `postgresql://postgres.${projectRef}:${encodeURIComponent(dbPassword)}@${poolerHost}:6543/postgres`
    ...
    fs.writeFileSync(tempFile, sql)
    try {
      execSync(`psql "${connString}" -f "${tempFile}"`, { stdio: 'inherit', timeout: 120000 })
      console.log('\n✓ Schema applied via psql')
    } catch (e) {
      console.log('psql connection failed, falling back to pg library...')
      psqlAvailable = false
    } finally {
      if (fs.existsSync(tempFile)) fs.unlinkSync(tempFile)
    }
  ...
  if (!psqlAvailable) {
    // Use custom DNS resolver via Cloudflare to resolve Supabase hostnames
    const dns = await import('dns')
    const resolver = new dns.promises.Resolver()
    resolver.setServers(['1.1.1.1', '8.8.8.8'])
    
    // Resolve pooler hostname to IPv4 via Cloudflare DNS
    const poolerHost = 'aws-0-eu-west-1.pooler.supabase.com'
    let poolerIp: string
    try {
      const [v4] = await resolver.resolve4(poolerHost)
      poolerIp = v4
      console.log(`  Pooler IPv4: ${poolerIp}`)
    } catch {
      try {
        const [v6] = await resolver.resolve6(poolerHost)
        poolerIp = v6
        console.log(`  Pooler IPv6: ${poolerIp}`)
      } catch (e: any) {
        console.error('Pooler DNS resolution failed:', e.message)
        throw e
      }
    }

    const { Pool } = await import('pg')
    // Try session mode (port 5432) first, then transaction mode (port 6543)
    const pool = new Pool({
      host: poolerIp,
      port: 5432,
      database: 'postgres',
